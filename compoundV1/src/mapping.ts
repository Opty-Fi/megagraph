...
export function handleAccrueInterest(event: AccrueInterestEvent): void {
  ...
  let cTokenDataEntity =  new CTokenData(
    event.transaction.hash.toHex() + "-" + event.logIndex.toString()
  )
  // cTokenDataEntity.totalCash = event.params.cashPrior
  // cTokenDataEntity.interestAccumulated = event.params.interestAccumulated
  cTokenDataEntity.borrowIndex = event.params.borrowIndex
  cTokenDataEntity.totalBorrows = event.params.totalBorrows
  ...
  cTokenDataEntity.totalCash = cTokenContract.getCash()
  cTokenDataEntity.exchangeRate = cTokenContract.exchangeRateStored()
  cTokenDataEntity.borrowRatePerBlock = cTokenContract.borrowRatePerBlock()
  cTokenDataEntity.totalReserves = cTokenContract.totalReserves()
